...
if [[ "$CONFIG_WRITTEN" == "true" ]]; then
    ...
    echo -e "  ${BOLD}Step 2:${NC} The mod will create ${CYAN}\$profile:PlayerTracker/config.cfg${NC}"
    echo -e "          automatically on first start. Edit it with your panel details:"
    echo ""
    echo -e "  ${CYAN}# PlayerTracker config"
    echo -e "  url=${PANEL_URL}/"
    echo -e "  api_key=${TRACKER_KEY}"
    echo -e "  track_path=api/tracker/track"
    echo -e "  event_path=api/tracker/event"
    echo -e "  update_interval=10${NC}"
    echo ""
    echo -e "  Or use the panel UI: ${BOLD}Tracker → ⚙ → Mod Setup${NC}"
fi
